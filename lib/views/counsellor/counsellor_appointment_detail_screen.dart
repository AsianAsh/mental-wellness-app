// // For counsellors to update and see detailed view of their appointments (including viewing member details who booked the appointment)
...
class _CounsellorAppointmentDetailScreenState
  ...
  void _updateAppointment() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      Navigator.pop(context, true); // Pass true to indicate the update
    }
  }
  ...
  Widget _buildLongDetail(String title, TextEditingController controller,
      {bool isEditable = true, String? Function(String?)? validator}) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget build(BuildContext context) {
    ...
    if (status == 'booked' || status == 'ongoing') {
      if (widget.appointment['meetingLink'].isNotEmpty &&
        ...
        meetingLinkText = 'Add Meeting URL';
      }
